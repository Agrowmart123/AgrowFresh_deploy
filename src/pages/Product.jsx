...
import { getCustomerProductById,getFarmerProductById, getWomenProductById,getAgriProductById } from "../services/api";
...
const productType = passedData?.type || "regular";
  const scrollRef = React.useRef(null);

useEffect(() => {
  if (passedData?.product) {
    setProduct(passedData.product);
    setSelectedImage(
      passedData.product?.imageUrls?.[0] ||
      passedData.product?.image ||
      null
    );
    setLoading(false);
  } else {
  ...
  item.name || item.product?.productName || "Item";

const getItemPrice = (item) =>
  item.price ||
  item.product?.details?.minPrice ||
  item.product?.price ||
  0;

// const normalizeProduct = (data, type) => {
const normalizeProduct = (data, type) => {
  if (!data) return null;
...
shortDescription:
  ...
  "",
      ...
      sellerAddress: data.shop?.shopAddress,

      shelfLife: data.shelfLife,
      ingredients: data.ingredients,
      packagingType: data.packagingType,

      vendorName: data.sellerName,
    };
  }
  ...
  if (type === "agri") {
    return {
      ...data,
      name: data.AgriproductName,
      productName: data.AgriproductName,
      description:
  data.Agridescription ||
  data.description ||
  "",
shortDescription:
  ...
  "",
      details: {
        minPrice: Number(data.Agriprice || 0),
        unit: data.Agriunit,
        quantity: data.Agriquantity,
      },
      ...
      shopName: data.vendor?.name,
      sellerAddress: "",

      shelfLife: "",
      brand: data.AgribrandName,
    };
  }
...
shortDescription:
  ...
  "",
    details: {
      minPrice: Number(data.details?.minPrice || 0),
    ...
    shelfLife: data.details?.shelfLife,
  };
};
 const fetchProduct = async () => {
  ...
  try {
    ...
    if (productType === "farmer") {
      res = await getFarmerProductById(id);
    } else if (productType === "women") {
      res = await getWomenProductById(id);
    } else if (productType === "agri") {
      res = await getAgriProductById(id);
    } else {
      res = await getCustomerProductById(id);
    }

    data = res?.data?.data || res?.data || null;

  const normalized = normalizeProduct(data, productType);
...
setSelectedImage(firstImage);
    ...
    if (data?.id) {
      try {
        ...
        setWishlisted(wRes?.data?.wishlisted || false);
      } catch (_) {}
    }

    // ✅ RELATED PRODUCTS (IMPORTANT 🔥)
    if (productType === "farmer" && data?.farmerId) {
      const res2 = await getProductsByFarmer(data.farmerId);
      setSimilarProducts(res2?.data || []);
    } 
    else if (productType === "regular" && data?.shop?.shopId) {
      const res2 = await getCustomerShopProducts(data.shop.shopId);
      setSimilarProducts(res2?.data || []);
    }
    else if (productType === "women") {
...
const handleAddToCart = async () => {
  ...
  console.log("PRODUCT FULL 👉", product);// 👈 ADD

  try {
    const payload = {
      productId: product.id,
   "productType": productType,
     getMerchantId: product.merchantId,
      quantity: 1,
    };
    ...
    console.log("ADD RESPONSE 👉", res.data); // 👈 ADD
...
{(product.unit || product.details?.unit || product.stockQuantity || product.details?.quantity) && (
  ...
  ₹{product.details?.minPrice || product.price || 0}
</span>
  {product.mrp &&
    product.mrp >
  ...
  className="flex-1 border border-[#68911a] text-[#68911a] py-3 rounded-lg font-semibold hover:bg-[#68911a] hover:text-white transition flex items-center justify-center gap-2"
>
